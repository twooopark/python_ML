...
## 4. 비지도학습 
  ...
  - k-means 클러스터 알고리즘
    ...
    - 적절한 클러스터의 개수는 어떻게 구할까?
      ...
      - tss를 클러스터들에 있어서 각 클러스터 중심점과 데이터의 거리의 합이라고 한다면, 클러스터의 개수를 늘릴수록, tss는 점점 작아질 것이다.(더 상세하게 나누기 때문에) 하지만, 클러스터의 개수가 너무 많으면, 각 클러스터의 특성을 나타내는데 낭비가 생겨나게 된다.(하나의 특징을 가진 군집을 둘로 나누는 낭비가 생길 수 있음) 그러므로 tss 값의 변화를 관찰하고, 클러스터의 개수가 늘어날 수록 급격히 줄어드는 tss의 기울기가 완화되는 변곡점(elbow)을 찾아 최적의 클러스터 개수를 구할 수 있다.
